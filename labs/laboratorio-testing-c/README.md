...
> R: Inicializa la cantidad de items del carrito en 0. Es importante llamarla para no operar sobre "basura" o valores residuales que hayan quedado en la memoria.
...
> R: Si, el test falló. Esperaba 700 pero obtuvo 350
...
> R: El bug estaba en la función carrito_total, linea 24 y dice total += c->items[i].precio; 

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: La línea deberia multiplicar el precio por la cantidad
...
> R: Faltaba la validación de límite. Debería usar el operador < (menor), específicamente c->cantidad < MAX_ITEMS
...
> R: Si, las lineas 29 y 30. No se ejecutaron porque el comando de cobertura corrió los tests unitarios y nosotros no escribimos ningún test unitario para esa función
...
> R: El unitario prueba una función aislada, mientras que el de integración prueba cómo interactúan varias funciones. El unitario detectó primero el bug de carrito_total
...
> R: En C se corrompe la memoria porque el lenguaje no verifica automáticamente los límites del array. Lenguajes como Java o Python manejan la memoria de forma segura y lanzan una excepción antes de que ocurra el desbordamiento
...
> R: Escribir tests permite automatizar la validación cada vez que cambiás algo y te obliga a pensar en los casos límite que a simple vista no se notan
...
> R: Pasó porque cualquier precio multiplicado por 1 da el mismo precio. Como la cantidad era 1, la falta de la multiplicación en la lógica original no alteró el resultado final
